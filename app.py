...
def authorization(plate_number):
    ...
    with engine.connect() as connection:
        query = text("SELECT plate_number FROM authorized_plates WHERE plate_number = :plate")
        result = connection.execute(query, {"plate": plate_number})
        row = result.fetchone()
...
# Streamlit UI
st.title("Vehicle Registration and Authentication System")
...
plate_number = st.text_input("Enter Vehicle Plate Number")
    ...
    if plate_number.strip():
        ...
        try:
            with engine.begin() as connection: 
                query = text("""
                    INSERT INTO authorized_plates (plate_number)
                    VALUES (:plate)
                """)
                connection.execute(query, {"plate": plate_number.strip()})

            st.success(f"Vehicle {plate_number} registered successfully.")

        except Exception as e:
            st.error(f"Error registering vehicle: {e}")

    else:
        st.error("Please enter a valid plate number.")